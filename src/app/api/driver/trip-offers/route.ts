    ...
    const trips =
      tripIds.length > 0
        ? await serviceClient
            .from("trips")
            .select(
              "id, customer_id, trip_type, status, pickup_label, pickup_address, destination_label, destination_address, passenger_count, luggage_count, estimated_price, created_at"
            )
            .in("id", tripIds)
        : { data: [] as any[] };
