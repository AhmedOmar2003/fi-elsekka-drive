    ...
    return (
        <div className="space-y-6">
            <section className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
                <SectionCard title="Active ride requests" subtitle="Trips waiting for dispatch or manual assignment">
                    <div className="space-y-4">
                            ...
                            <div key={trip.id} className="rounded-[28px] border border-white/10 bg-white/[0.03] p-4">
                                <div className="flex flex-wrap items-center justify-between gap-3">
                                    <div>
                                        ...
                                        <p className="mt-1 text-xs text-white/45">{new Date(trip.createdAt).toLocaleString("en-GB")}</p>
                                    </div>
                                    <StatusBadge status={trip.status} />
                                </div>
                                <div className="mt-4 grid gap-3 md:grid-cols-2">
                                    <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-3">
                                        <p className="text-xs uppercase tracking-[0.2em] text-white/35">From</p>
                                        <p className="mt-2 text-sm text-white/75">{trip.pickup}</p>
                                    </div>
                                    <div className="rounded-2xl border border-white/10 bg-white/[0.025] p-3">
                                        <p className="text-xs uppercase tracking-[0.2em] text-white/35">To</p>
                        ...
                        {board.activeTrips.length === 0 ? <p className="text-sm text-white/45">No active requests in dispatch queue.</p> : null}
                    </div>
                </SectionCard>

                <div className="space-y-6">
                    <SectionCard title="Available drivers" subtitle="Online captains and their primary vehicles">
                            ...
                            {board.availableDrivers.map((driver) => (
                                <Link key={driver.id} href={`/admin/drivers/${driver.id}`} className="flex items-center justify-between rounded-[24px] border border-white/10 bg-white/[0.03] p-4 transition hover:border-primary/25 hover:bg-white/[0.05]">
                                    <div>
                                        ...
                                        <p className="mt-1 text-xs text-white/55">{driver.vehicleLabel || "No primary vehicle"}</p>
                                    </div>
                    ...
                    <SectionCard title="Map-ready dispatch canvas" subtitle="Placeholder for future live map, ETA, and nearest driver overlays">
                        <div className="flex min-h-[22rem] items-center justify-center rounded-[28px] border border-dashed border-primary/25 bg-[radial-gradient(circle_at_center,rgba(20,148,111,0.12),transparent_45%)] text-center">
                            <div>
                                <p className="text-lg font-black">Live map placeholder</p>
                                <p className="mt-2 max-w-sm text-sm leading-7 text-white/55">
                                    This area is ready to connect later with live driver locations, route lines, ETA cards, and dispatch clustering from Supabase Realtime + maps SDK.
                                </p>
                            </div>
                        </div>
                    </SectionCard>
